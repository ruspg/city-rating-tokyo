...
| #73 | — | **FlyTo cleanup:** hide halo ring + close open popup during flyTo animation (uses `isFlyingRef` guard). |
| #76 | — | **iOS Safari fixes:** search input `text-base` (16px) prevents auto-zoom, search pill `right-24` clears Heatmap button, root `overflow-x-hidden`, zoom buttons safe-area-aware `calc(80px + env(safe-area-inset-bottom))`. |
